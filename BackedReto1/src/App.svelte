...
<script> 
	...
	async function getNewMiningGame() {
		...
		const response = await fetch("https://w4390n0i.directus.app/items/Games", requestOptions)
	...
	function incrementCount2() {
		...
		const response = fetch("https://w4390n0i.directus.app/items/Games/"+gameId, requestOptions)
...
<main>
	...
	{:then items} 
		...
		{setEmojiValues (items.data[Math.floor(Math.random()*items.data.length)].Emoji)}
	{:catch error}
...
<button on:click={incrementCount2}>
	⛏️
</button>
 
	<div class="footer"> 
		<p> Solucion para Reto de <a href="https://github.com/malandrinersdev/backend-de-la-suerte"> Backend de la suerte</a> | Visita la comunidad <a href="https://danielprimo.io">DanielPrimo.io</a> para otros retos en pantuflas como este | Done with ❤️ </p>
	</div>
